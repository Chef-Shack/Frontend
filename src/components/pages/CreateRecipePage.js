...
const CreateRecipePage = (props) => {
  ...
  async function handleSubmit(event) {
    ...
    if(confirm) {
      ...
      await axios.get(`https://chefshack-backend.herokuapp.com/members/get_user_by_name/${window.localStorage.getItem('username')}`).then((res) => {
        id = res.data.id
      }).catch((err) => {
        console.log(err)
      })
      ...
      if(id != null) {
        ...
        axios.post("https://chefshack-backend.herokuapp.com/recipes/create_recipe", bodyFormData).then((res) => {
